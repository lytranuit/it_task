            ...
            <div class="mt-0 mb-3 d-flex">
                ...
                <div class="ml-auto">
                    <span class="p-buttonset">
                        <Button label="Chưa hoàn thành" size="small" :outlined="type_2 != 'chuahoanthanh'"
                            @click="viewChuaHoanThanh" />
                        <Button label="Quá hạn" size="small" :outlined="type_2 != 'quahan'" @click="viewQuahan" />
                    </span>
                    <span class="p-buttonset ml-3">
                        <Button label="Việc của tôi" size="small" :outlined="type != 'mytask'" @click="viewMyTask" />
                        <Button label="Việc tôi giao" size="small" :outlined="type != 'myassign'" @click="viewMyAssgin" />
                    </span>
                </div>
            </div>
            <EjsGantt :key="key" ref='gantt' :dataSource="taskList" :treeColumnIndex='1' id="GanttContainer"
                :taskFields="taskFields" :editSettings="editSettings" :toolbar="toolbar" :toolbarClick="toolbarClick"
                :allowSelection="true" :actionComplete="actionComplete" :includeWeekend="true" :columns="columns"
                :splitterSettings="splitterSettings" :timelineSettings="timelineSettings" height="600px"
                ...
                <EjsGanttColumns>
                    ...
                    <EjsGanttColumn field='project' headerText='Dự án' :template="'projectTemplate'"></EjsGanttColumn>
                ...
                <template v-slot:rightLabelTemplate="{ data }">
                    <div v-if="data.taskData.assignees">
                        <AvatarGroup>
                            <Avatar v-for="assignee in data.taskData.assignees" :image="assignee.user.image_url"
                                shape="circle" :title="assignee.user.fullName" />
                ...
                <template v-slot:projectTemplate="{ data }">
                    {{ data.taskData.project.name }}
                </template>
...
const dayWorkingTime = ref([{ from: 0, to: 23 }])
...
const viewChuaHoanThanh = () => {
    if (type_2.value == "chuahoanthanh")
        type_2.value = null;
    else
        type_2.value = "chuahoanthanh";
    refresh();
}
const viewQuahan = () => {
    if (type_2.value == "quahan")
        type_2.value = null;
    else
        type_2.value = "quahan";
    refresh();
...
const toolbar = ref([{ text: 'Gantt', tooltipText: 'gantt', id: 'viewGantt', align: 'Right' }]);
const toolbarClick = (args) => {
    if (args.item.id === 'viewGantt') {
        toolbar.value = ["ZoomIn", "ZoomOut", "ZoomToFit", "PrevTimeSpan", "NextTimeSpan", { text: 'Table', tooltipText: 'Table', id: 'viewTable', align: 'Right' }];
        splitterSettings.value.position = "0%";
        key.value = rand();
    } else if (args.item.id === 'viewTable') {
        toolbar.value = [{ text: 'Gantt', tooltipText: 'gantt', id: 'viewGantt', align: 'Right' }];
        splitterSettings.value.position = "100%";
        key.value = rand();
    }
}
// const actionBegin = (e) => {
